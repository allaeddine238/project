...
// Meals screen.
// This page handles traditional meal search, parser-based logging, and today's meal summary.
...
// Gives each food card a predictable default amount and mode before the user edits it.
...
// Traditional meals come from a content dataset, so this reshapes them into the format used by the screen.
...
function calculateNutrition(item, state) {
  // Converts per-100g nutrition into the live amount the user selected on the card.
...
export function MealsScreen() {
  ...
  const renderFoodCard = (item, accent = 'var(--em)') => {
    // One renderer keeps database meals and search matches looking identical.
