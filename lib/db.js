...
// Screens and provider code call these helpers instead of writing Supabase queries directly.
import { supabase, isSupabaseConfigured } from '@/lib/supabase/client'
import { todayKey } from '@/lib/utils'

const safe = async (fn, fallback = { data: null, error: { message: 'Supabase not configured' } }) => {
  // This wrapper keeps a missing setup or query failure from crashing the whole UI.
...
export const getSession = async () => {
  // Session lookup is separated because the provider uses it during app boot.
...
// Purchase requests are stored so token or plan activity can be tracked later.
